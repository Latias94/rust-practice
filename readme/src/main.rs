...
fn main() -> Result<()> {
    ...
    let leetcode_solution_question_id_set = leetcode_solutions
        .iter()
        .enumerate()
        .map(|(_, v)| (v.question_id))
        .collect::<HashSet<i32>>();

    let mut readme_leetcode_question = leetcode_questions
        .into_iter()
        .filter(|question| leetcode_solution_question_id_set.contains(&question.id))
        .collect::<Vec<LeetcodeQuestion>>();
    readme_leetcode_question.sort_by_key(|x| x.id);

    let readme_text: String = ReadmeContext::new(
        readme_leetcode_question,
